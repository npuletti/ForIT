...
#' superimposed as a light coloured line. Function output is a \code{ggplot} object that can be used
#' by its self or as a backgrund on top of which the user can plot his/her data to
...
#' }
#' (** \code{compute_grid0()} - is an internal function expotred for the sake of
...
#'            \item \code{FALSE} = variance for the 'estimate of an avarage' value
#'           Default \code{ie.Va = FALSE}}
#' @param cv.ul (optional) Numeric. Cutoff CV level for the plot. Defaults to 0.1
#' @param fixed (optional) Logical. Contur plot breaks:
#'          \itemize{
...
#'          plot with better resolution but increases consitently computation time
#'          (see \code{INFCaccuracyPlot0()}). To reduce coputation time, the step
#'          defaults to 5.
  ...
  if (plot.est) {
    ...
    cvg <- cvg +
      ...
      metR::geom_text_contour(
        ggplot2::aes(z = est),
        size = 3,
        label.placement = metR::label_placement_fraction(frac = 0.5)
      )
  }
